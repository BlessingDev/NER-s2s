...
class T5ForConditionalGeneration(T5PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: T5Config, inerd_version=1):
        ...
        if inerd_version == 1:
            self.inerd_decoder = inerd_decoding
        elif inerd_version == 2:
            self.inerd_decoder = inerd2_decoding
...
class T5ForConditionalGenerationTrain(T5PreTrainedModel, GenerationMixin):
    _keys_to_ignore_on_load_unexpected = [
        "decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight",
    ]
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight", "lm_head.weight"]
