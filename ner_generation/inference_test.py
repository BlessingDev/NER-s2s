...
def main(args):
    ...
    if args.prompt_type == "inerd":
        ...
        saved_model.initialize_inerd(
            line_break_indicator=tokenizer.encode(LINE_BREAK_INDICATOR),
    ...
    def generate_predictions_s2s(sentence, entity_list):
        """Generates NER JSON for a batch of sentences."""
        # Prepare inputs with the prefix
        inputs_with_prefix = []

        if args.prompt_type == "inerd":
            inputs_with_prefix.append(PREFIX_INERD.format(entity_types=" ".join(entity_list), sentence=sentence))
        else:
            inputs_with_prefix.append(PREFIX.format(entity_types=" ".join(entity_list), sentence=sentence))
    ...
    generated_results = generate_prediction_func(args.sentence, entity_types_dict[args.dataset_name])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sentence",
        type=str,
        default="The final stage in the development of the Skyfox was the production of a model with tricycle landing gear to better cater for the pilot training market ."
    )

    #args = parser.parse_args()
    args = parser.parse_args([
        "--model_checkpoint", "/workspace/model_dir/generation/flan-t5-base/ner-inerd-conll-fp32-w1e3-lr4e5/final_model",
        "--batch_size", "32",
        "--dataset_name", "conll2003",
        "--data_file", "/workspace/datas/conll2003/testb.preprocessed.csv",
        "--prompt_type", "inerd",
        "--sentence", "ORE - Maritime Queen 70,000 tonnes Dampier / Kaohsiung 20-30/12 $ 5.25 fio 35,000 / 30,000 China Steel ."
    ])
